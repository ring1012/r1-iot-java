    ...
    public JsonNode fetchMusics(MusicAiResp musicAiResp, Device device) {
        try {
            ...
            if(StringUtils.hasLength(globalConfig.getYtdlpEndpoint())){
                ...
                resp = restTemplate.getForEntity(baseUrl+"search/music?keyword="+keyword, JsonNode.class).getBody();
            }else{

                ObjectNode req = (ObjectNode) objectMapper.readTree(SAMPLE_PAYLOAD);
                req.put("query", keyword);

                 resp = restTemplate.postForEntity(MUSIC_SEARCH, req, JsonNode.class).getBody();
            }
            ...
            ArrayNode contents = (ArrayNode) resp.get("contents").get("tabbedSearchResultsRenderer").get("tabs").get(0)
